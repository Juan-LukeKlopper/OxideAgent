...
fn test_global_tool_permissions_save_load() {
    ...
    assert!(save_result.is_ok(), "save() failed with error: {:?}", save_result.err());
    ...
    assert!(content_result.is_ok(), "Failed to read file: {:?}", content_result.err());
    let content = content_result.unwrap();
    assert!(!content.trim().is_empty());
    
    // Test loading permissions
    let load_result = GlobalToolPermissions::load_from_path(&test_file_path);
    assert!(load_result.is_ok(), "load() failed with error: {:?}", load_result.err());
