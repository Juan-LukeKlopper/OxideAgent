...
fn test_config_file_examples() {
    ...
    assert_eq!(strava_server.description.as_ref().unwrap(), "Strava MCP server for fitness data");
    assert_eq!(strava_server.auto_start, Some(true));
    
    match &strava_server.server_type {
        McpServerType::Npm { package, command, .. } => {
    ...
    assert_eq!(github_server.description.as_ref().unwrap(), "GitHub MCP server");
    
    match &github_server.server_type {
        McpServerType::Remote { url, access_token, .. } => {
