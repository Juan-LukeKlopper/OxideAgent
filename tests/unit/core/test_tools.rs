...
use OxideAgent::core::mocks::{MockFileSystem, MockShellExecutor, MockWriteFileTool, MockReadFileTool, MockRunShellCommandTool};
...
fn test_tool_registry_definitions_with_profiles() {
    ...
    let shell_tool_names: Vec<String> = shell_tools.iter().map(|t| t.function.name.clone()).collect();
