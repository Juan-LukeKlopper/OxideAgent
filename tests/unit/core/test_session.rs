...
fn test_session_manager_save_and_load() {
    ...
    assert!(save_result.is_ok(), "Failed to save state: {:?}", save_result.err());
    ...
    assert!(load_result.is_ok(), "Failed to load state: {:?}", load_result.err());
...
fn test_session_manager_load_nonexistent() {
    ...
    assert!(result.is_ok(), "Loading nonexistent should succeed with None: {:?}", result.err());
...
fn test_session_manager_load_empty_file() {
    ...
    assert!(write_result.is_ok(), "Failed to write test file: {:?}", write_result.err());
    
    // Try to load from the empty file
    let result = SessionManager::load_state(&temp_file);
    assert!(result.is_ok(), "Failed to load empty file: {:?}", result.err());
...
fn test_session_manager_load_invalid_json() {
    ...
    let result = SessionManager::load_state(&temp_file);
    assert!(result.is_ok(), "Failed to load invalid JSON: {:?}", result.err());
...
fn test_session_manager_list_sessions_default() {
    ...
    assert!(save_result.is_ok(), "Failed to save default session: {:?}", save_result.err());
    
    // List sessions
    let sessions_result = SessionManager::list_sessions();
    assert!(sessions_result.is_ok(), "Failed to list sessions: {:?}", sessions_result.err());
...
fn test_session_manager_list_sessions_named() {
    ...
    assert!(save_result.is_ok(), "Failed to save named session: {:?}", save_result.err());
...
fn test_session_manager_get_session_filename() {
    ...
    assert_eq!(SessionManager::get_session_filename(Some("test_session")), "session_test_session.json");
}