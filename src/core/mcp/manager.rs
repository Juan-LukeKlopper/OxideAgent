...
pub struct McpConnectionRegistry {
    pub connections: Arc<RwLock<HashMap<ConnectionId, Arc<Mutex<McpConnectionType>>>>>,
}
...
impl McpConnectionRegistry {
    ...
    pub async fn add_stdio_connection(&self, id: ConnectionId, connection: StdioMcpConnection) {
        ...
        connections.insert(
            id,
            Arc::new(Mutex::new(McpConnectionType::Stdio(connection))),
        );
    }
    ...
    pub async fn add_http_connection(&self, id: ConnectionId, connection: HttpMcpConnection) {
        ...
        connections.insert(
            id,
            Arc::new(Mutex::new(McpConnectionType::Http(connection))),
        );
    }

    pub async fn get_connection(&self, id: &ConnectionId) -> Option<Arc<Mutex<McpConnectionType>>> {
...
pub struct McpManager {
    pub registry: McpConnectionRegistry,
}
...
impl McpManager {
    pub fn new() -> Self {
        init_mcp_registry();
        Self {
            registry: (*get_mcp_registry()).clone(),
        }
    }
...
impl McpManager {
    pub fn get_registry(&self) -> &McpConnectionRegistry {
        &self.registry
    }
}
